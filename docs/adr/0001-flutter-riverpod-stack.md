...
Linux/Arch with an Android/Pixel device. iOS support is deferred (see private
ADR 0003). A cross-platform solution was required to avoid maintaining separate
native codebases when other platforms are added later.
...
- iOS builds require macOS and Xcode — deferred until a Mac, Apple Developer
  ...
  solution)
- Linux desktop is not a current target
